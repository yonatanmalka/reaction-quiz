            ...
            <div className="flex flex-col gap-[15px] mt-[30px]">
                {list.map((item,index) => (
                    <button
                        ...
                        key={index} className={`rounded-[10px] px-[15px] py-[30px] relative ${selected === index ? 'bg-opacity-30' : ''} ${selected === index ? 'bg-yellow-400' : 'bg-[#F5F5F5]'} border-[1px] ${selected === index ? 'border-[#F9B22D]' : 'border-[#979797]'} border-[#979797] flex flex-col w-[100%] justify-center items-center`}
                    >
                        <Image
                            src={item.image}
                            alt={item.name}

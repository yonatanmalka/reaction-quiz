...
const Question1:React.FC<QuestionProps> = ({handleClick}) => {
    ...
    return(
        <div className="relative">
            ...
            <div className="mt-[22px] flex flex-col text-center">
            ...
            <div className="flex justify-center">
                <h1 className="text-[#979797] text-[14px] mt-[10px] font-normal">FIRST, SELECT YOUR <span className="font-bold">MAIN GOAL</span></h1>
            </div>
            <h1 className="text-[#000] text-[12px] font-medium text-center mt-[10px]">
                1-MINUTE
            </h1>
            <div style={{marginTop:25}}>
                <div className="grid grid-cols-2 gap-x-[15px] gap-y-[12px] mt-[15px]">
                    {data.map((item,index) => (
                        <button
                            key={index}
                            className={`rounded-[12px] ${selected === index ? 'bg-opacity-30' : ''} ${selected === index ? 'bg-yellow-400' : 'bg-[#F5F5F5]'} border-[1px] ${selected === index ? 'border-[#F9B22D]' : 'border-[#979797]'} border-[#979797] flex flex-col  gap-[3px] md:gap-[5px] py-[20px] md:py-[18px] justify-center relative items-center`}
            ...
            <div className="">
            <button onClick={handleNextClick} className="uppercase  h-[40px] flex items-center justify-center mt-[22px] bg-[#F9B22D] w-[100%] rounded-[24px] text-[12px] md:text-[16px] font-semibold leading-10 tracking-tight text-[#000]">
                Next
            </button>
            <div className="text-center text-[#606060] text-[12px]  mt-[7px]">
